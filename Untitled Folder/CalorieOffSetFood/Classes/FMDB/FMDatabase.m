    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (busyRetryTimeout && (numberOfRetries++ > busyRetryTimeout)) {
                PrintLog(@"%s:%d", __FUNCTION__, __LINE__);
                PrintLog(@"Database busy, unable to close");
                return NO;
            }
            ...
            if (!triedFinalizingOpenStatements) {
                ...
                while ((pStmt = sqlite3_next_stmt(db, 0x00)) !=0) {
                    PrintLog(@"Closing leaked statement");
                    sqlite3_finalize(pStmt);
                }
            }
        }
        else if (SQLITE_OK != rc) {
            PrintLog(@"error closing!: %d", rc);
        }
    }
    while (retry);
...
- (BOOL)rekey:(NSString*)key {
    ...
    if (rc != SQLITE_OK) {
        PrintLog(@"error on rekey: %d", rc);
        PrintLog(@"%@", [self lastErrorMessage]);
    }
...
- (void)warnInUse {
    PrintLog(@"The FMDatabase %@ is currently in use.", self);
...
- (BOOL)databaseExists {
    ...
    if (!db) {
        ...
        PrintLog(@"The FMDatabase %@ is not open.", self);
...
- (FMResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (traceExecution && sql) {
        PrintLog(@"%@ executeQuery: %@", self, sql);
    }
    ...
    if (!pStmt) {
        do {
            ...
            if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
                ...
                if (busyRetryTimeout && (numberOfRetries++ > busyRetryTimeout)) {
                    PrintLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [self databasePath]);
                    PrintLog(@"Database busy");
                    sqlite3_finalize(pStmt);
                    [self setInUse:NO];
                    return nil;
                }
                ...
                if (logsErrors) {
                    PrintLog(@"DB Error: %d \"%@\"", [self lastErrorCode], [self lastErrorMessage]);
                    PrintLog(@"DB Query: %@", sql);
#ifndef NS_BLOCK_ASSERTIONS
                    if (crashOnErrors) {
    ...
    if (idx != queryCount) {
        PrintLog(@"Error: the bind count is not correct for the # of variables (executeQuery)");
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return nil;
    }
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orVAList:(va_list)args {
    ...
    if (traceExecution && sql) {
        PrintLog(@"%@ executeUpdate: %@", self, sql);
    }
    ...
    if (idx != queryCount) {
        PrintLog(@"Error: the bind count is not correct for the # of variables (%@) (executeUpdate)", sql);
        sqlite3_finalize(pStmt);
        [self setInUse:NO];
        return NO;
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    PrintLog(@"Unexpected result from sqlite3_reset (%d) eu", rc);
                }
            }
            usleep(20);
            ...
            PrintLog(@"Error calling sqlite3_step (%d: %s) SQLITE_ERROR", rc, sqlite3_errmsg(db));
            PrintLog(@"DB Query: %@", sql);
        }
        else if (SQLITE_MISUSE == rc) {
            // uh oh.
            PrintLog(@"Error calling sqlite3_step (%d: %s) SQLITE_MISUSE", rc, sqlite3_errmsg(db));
            PrintLog(@"DB Query: %@", sql);
        }
        else {
            // wtf?
            PrintLog(@"Unknown error calling sqlite3_step (%d: %s) eu", rc, sqlite3_errmsg(db));
            PrintLog(@"DB Query: %@", sql);
        }
