...
- (NSDictionary *)resultDict {
    ...
    if (num_cols > 0) {
        ...
        PrintLog(@"Warning: There seem to be no columns in this set.");
    }
    ...
    do {
        ...
        if (SQLITE_BUSY == rc || SQLITE_LOCKED == rc) {
            ...
            if (SQLITE_LOCKED == rc) {
                ...
                if (rc != SQLITE_LOCKED) {
                    PrintLog(@"Unexpected result from sqlite3_reset (%d) rs", rc);
                }
            }
            usleep(20);
            ...
            if ([parentDB busyRetryTimeout] && (numberOfRetries++ > [parentDB busyRetryTimeout])) {
                ...
                PrintLog(@"%s:%d Database busy (%@)", __FUNCTION__, __LINE__, [parentDB databasePath]);
            ...
            PrintLog(@"Error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        } 
            ...
            // wtf?
            PrintLog(@"Unknown error calling sqlite3_step (%d: %s) rs", rc, sqlite3_errmsg([parentDB sqliteHandle]));
            break;
        }
...
- (int)columnIndexForName:(NSString*)columnName {
    ...
    PrintLog(@"Warning: I could not find the column named '%@'.", columnName);
...
- (NSString*)stringForColumnIndex:(int)columnIdx {
    ...
    return [NSString stringWithUTF8String:(char *)sqlite3_column_text(statement.statement, columnIdx)];
}
